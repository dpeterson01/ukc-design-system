...
| Logo SVG masters | `brand/logos/` (source of truth in `ukc-data`; synced into `assets/logos/`) | Primary lockup, stacked lockup, three shields (combined / SJB / IC), three standalone icons (lamb / Marian M / circle), plus the cross element. Each comes in fullcolor / mono-navy / mono-white / reverse variants. |
...
└── ui_kits/                   ← per-surface kits
    ...
    └── parish_letterhead/     ← print letterhead (HTML → PDF + DOCX template)
```
...
| **Three-mark lockup (icons only)** | `parish-fullcolor_three-mark-lockup.svg` | Lamb · Cross · Marian M, no text. Use when parish names appear nearby in the layout. |
...
| `assets/logos/` | Full SVG logo system: primary lockup, three-mark lockup (icons only), stacked lockup, circle icon, SJB lamb, IC Marian M, cross, combined shield, individual shields — each in `fullcolor` / `mono-navy` / `mono-white` / `reverse` variants where applicable. |
...
| `ui_kits/parish_letterhead/` | Parish letterhead template. `letterhead.html` — open in browser, fill fields, print to PDF. `letterhead.css` — print styles. The matching `letterhead.docx` (Word template) is generated by `scripts/build-letterhead-docx.js` and stored in `ukc-data/assets/brand/templates/`. |

## Caveats and open items

- **Logo SVGs are synced from the ukc-data brand vault, which is their source of truth.** They live canonically in `ukc-data/assets/brand/logos/` (alongside the `.ai` masters, print PDFs, and raster exports); `assets/logos/` here holds the flattened runtime subset that the UI kits and renderers consume. Re-sync with `scripts/sync-logos.sh` whenever the vault changes. Last synced 2026-07-27: added three-mark-lockup (all four color variants); removed stale `parish-mono-white_three-mark.svg` (superseded by vault version) and three SJB shield mono/reverse variants deleted from the vault on 2026-07-17. One mark remains DS-native and not present in the vault — `parish-reverse_combined-shield.svg` (preview card) — so the sync script leaves it untouched. If the May 2026 committee lockup proportions (Lamb 56%, Cross 103%, full Marian Monogram 100%, all bottom-aligned) were not yet applied when the vault SVGs were exported, spot-check the primary/stacked lockups against the hand-composed preview cards, which follow the committee spec.
...
- **Letterhead is built** (`ui_kits/parish_letterhead/`). Both HTML (browser-fill → PDF) and DOCX (Word template) formats are available. DOCX regenerated via `scripts/build-letterhead-docx.js`; output stored in `ukc-data/assets/brand/templates/letterhead.docx`. Email/newsletter templates are not yet built.
