...
- **Logo SVGs are synced from the ukc-data brand vault, which is their source of truth.** They live canonically in `ukc-data/assets/brand/logos/` (alongside the `.ai` masters, print PDFs, and raster exports); `assets/logos/` here holds the flattened runtime subset that the UI kits and renderers consume. Last synced 2026-07-18 to the 2026-07-17 twelve-star Marian Monogram artwork. Two marks are DS-native and not present in the vault — `parish-mono-white_three-mark.svg` (newsletter header) and `parish-reverse_combined-shield.svg` (preview card) — so re-syncs must preserve them. If the May 2026 committee lockup proportions (Lamb 56%, Cross 103%, full Marian Monogram 100%, all bottom-aligned) were not yet applied when the vault SVGs were exported, spot-check the primary/stacked lockups against the hand-composed preview cards, which follow the committee spec.
